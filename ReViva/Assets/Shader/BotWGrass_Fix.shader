Shader "Custom/BotWGrass_Fix"
{
	...
	SubShader
	{
		...
		HLSLINCLUDE
			...
			float tessellationEdgeFactor_distanceTest(VertexOutput vert0, VertexOutput vert1)
			{
				float3 v0 = vert0.vertex.xyz;
				float3 v1 = vert1.vertex.xyz;
				float edgeLength = distance(v0, v1);
			...
			{
				VertexOutput i = (VertexOutput)0;

				// All three control points of a patch belong to the same draw
				// instance, so the stereo eye index is the same for all of them.
				UNITY_TRANSFER_VERTEX_OUTPUT_STEREO(patch[0], i);
			...
			GeomData TransformGeomToClip(float3 pos, float3 offset, float3x3 transformationMatrix, float2 uv, VertexOutput source)
			{
				GeomData o = (GeomData)0;

				UNITY_TRANSFER_VERTEX_OUTPUT_STEREO(source, o);
