Shader "Custom/StylizedLeaves_Teal_Corrigido"
{
    Properties
    {
        ...
        [Header(Base Colors)]
        _MainColor ("Main Color", Color) = (0.12, 0.62, 0.42, 1)
        _ShadowColor ("Shadow Color", Color) = (0.02, 0.18, 0.28, 1)
        _HighlightColor ("Highlight Color", Color) = (0.40, 0.92, 0.55, 1)
        ...
        [Header(Z Gradient)]
        _ZGradientColor ("Z Gradient Color (Bottom)", Color) = (0.02, 0.08, 0.22, 1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // FIX VR: Cull Back em vez de Off para evitar z-fighting stereo
            // Use Cull Off apenas se precisar ver os dois lados da folha
            Cull Off
            ZWrite On
            ZTest LEqual

            HLSLPROGRAM
            // FIX VR: pragmas obrigatórias para single-pass instanced
            #pragma vertex vert
            ...
            // FIX VR: suporte a single-pass stereo instancing
            ...
            struct Attributes
            {
                ...
                float2 uv         : TEXCOORD0;
                // FIX VR: necessário para instancing stereo
            ...
            struct Varyings
            {
                ...
                // FIX VR: necessário para passar stereo index entre passes
            ...
            {
                Varyings OUT;
                // FIX VR: inicializa instance ID e stereo index
            ...
            {
                // FIX VR: transfere stereo index para o fragment
                UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(IN);

                half4 texSample = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, IN.uv);
                clip(texSample.a - _AlphaCutoff);

                // FIX VR: GetMainLight sem shadow coord direto — usa macro segura
