...
public class Degrais : MonoBehaviour
{
    ...
    public void GerarDegraus()
    {
        ...
        while (y < alturaFinal && seguranca < maxPedras)
        {
            ...
            // =========================================================
            // CASA ROOT (adicionado à versão antiga)
            // =========================================================
            // Somente a plataforma final usa o Casa_Root como ponto de
            // referência. O Casa_Root NÃO é movido nem alterado — apenas
            // calculamos onde o prefab deve ser instanciado para que o
            // Casa_Root fique exatamente em 'posicaoPedra'. Para as pedras
            // normais e para o ponto de descanso, nada muda: continuam
            // instanciadas normalmente pelo próprio pivô do prefab.
            // =========================================================
            Vector3 posicaoInstanciacao = posicaoPedra;

            if (plataformaFinalGerada && prefabEscolhido == plataformaFinal)
            {
                posicaoInstanciacao = CalcularPosicaoInstanciacaoPeloCasaRoot(
                    prefabEscolhido,
                    posicaoPedra,
                    rot
                );
            ...
            GameObject instancia = Instantiate(prefabEscolhido, posicaoInstanciacao, rot, transform);
            index++;
        ...
        if (!plataformaFinalGerada && teveAlgumaPedra)
        {
            ...
            if (prefabFinalFallback != null)
            {
                ...
                Quaternion rotFallback = prefabFinalFallback.transform.rotation;

                // Mesma lógica de Casa_Root também vale na rede de segurança,
                // já que aqui também estamos instanciando a plataforma final.
                Vector3 posicaoInstanciacaoFallback = CalcularPosicaoInstanciacaoPeloCasaRoot(
                    prefabFinalFallback,
                    posFallback,
                    rotFallback
                );

                Instantiate(prefabFinalFallback, posicaoInstanciacaoFallback, rotFallback, transform);
                plataformaFinalGerada = true;
    ...
    // na posição desejada.
    // =========================================================
    Vector3 CalcularPosicaoInstanciacaoPeloCasaRoot(GameObject prefab, Vector3 posicaoAlvo, Quaternion rotacao)
    {
        if (prefab == null)
            return posicaoAlvo;
        ...
        Vector3 casaRootLocal = prefab.transform.InverseTransformPoint(casaRoot.position);

        // Converte o deslocamento local do Casa_Root
        // para o espaço do mundo.
        Vector3 deslocamentoCasaRootNoMundo = rotacao * Vector3.Scale(prefab.transform.localScale, casaRootLocal);

        // O prefab nasce deslocado para que o Casa_Root
        // fique exatamente na posição desejada.
        return posicaoAlvo - deslocamentoCasaRootNoMundo;
    }
